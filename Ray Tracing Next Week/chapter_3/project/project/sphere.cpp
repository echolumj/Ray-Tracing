...
{
	vec3 offset = vec3(_radius, _radius, _radius);
	_aabb = aabb(_centerStart - offset, _centerStart + offset);
}
...
{
	vec3 offset    = vec3(_radius, _radius, _radius);
	auto bboxStart = aabb(_centerStart - offset, _centerEnd + offset);
	auto bboxEnd   = aabb(_centerEnd - offset, _centerEnd + offset);
	_aabb = aabb(bboxStart, bboxEnd);
}
